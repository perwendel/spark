...
public class SparkAPI {
    ...
    public synchronized void staticFileLocation(String folder) {
        ...
        if (!servletStaticLocationSet) {
            if (runFromServlet) {
                SparkFilter.configureStaticResources(FileFolder);
    ...
    private boolean hasMultipleHandlers() {
        return FileFolder != null || externalStaticFileFolder != null;
    }
     ...
     */
    private RouteImpl wrap(final String path, final Route route) {
     ...
     * @return the wrapped route
     */
    private RouteImpl wrap(final String path, String acceptType, final Route route) {
        if (acceptType == null) {
     ...
     * @return the wrapped route
     */
    private FilterImpl wrap(final String path, final Filter filter) {
     ...
     * @return the wrapped route
     */
    private FilterImpl wrap(final String path, String acceptType, final Filter filter) {
        if (acceptType == null) {
            acceptType = DEFAULT_ACCEPT_TYPE;
        }
    ...
    private void addFilter(String httpMethod, FilterImpl filter) {
        init();
