...
public class SparkTestUtil {
    ...
    private UrlResponse doMethod(String requestMethod, String path, String body, boolean secureConnection)
            throws Exception {
        URL url = new URL((secureConnection ? "https" : "http" )+ "://localhost:" + port + path);
    ...
    /**
     * Convenience method to use own truststore on SSL Sockets.  Will default to the self signed
     * keystore provided in resources, but will respect
     *
    ...
    /**
     * Return JVM param set truststore location, or keystore location if not set.
     * if keystore not set either, returns default
     *
